        ...
        .then((response) => {
            ...
            if (response.intent.displayName === "Take_appointment_phase1") {
                ...
                if (response.allRequiredParamsPresent) {
                    ...
                    UserModel.find({ speciality: response.queryText })
                        .then((results) => {
                            const p = new Promise((resolve, reject) => {
                                results.map((res) => {
                                    Location.findById(res.location).then(
                                        (result) => {
                            ...
                            p.then((result) => {
                                doc = DevaHealthAssistant;
                                if (DevaHealthAssistant !== null) {
                                    UserModel.findById(result).then((res) => {
                                        messageReceived =
                                            "The nearest doctor to your location is " +
                                            res.firstname +
                                            " " +
                                            res.lastname +
                                            ", please pick a date by the datepicker.";
                                        ChatTalks.create({
                                            messageSent,
                                            messageReceived,
                                            date,
                ...
                response.intent.displayName === "Take_appointment_phase2"
            ) {
                console.log("Phase2\n");
                var test = true;
                if (doc == null) {
                    messageReceived =
                        "please choose a doctor before picking a date";
                    ChatTalks.create({
                        messageSent,
                        messageReceived,
                        date,
                ...
                if (doc !== null) {
                    ...
                    const pr = new Promise((resolve, reject) => {
                        resolve(Appointment.find({ User: doc }));
                    });
                    pr.then((result) => {
                        ...
                        prom.then((res) => {
                            if (res === true) {
                                ...
                                messageReceived =
                                    "Appointment confirmed at  you will get email with all details, and you can always cancel your appointment.";
                                ...
                                Appointment.create({
                                    ...
                                    EndDate,
                                    User,
                                    DoctorName: docname + " " + doclastname,
                                });
                                Appointment.create({
                                    Firstname,
                                    Email,
                                ...
                                messageReceived =
                                    "Date unavailable please pick another date";
                                ChatTalks.create({
                                    messageSent,
                                    messageReceived,
                                    date,
